...
function buildScenarios(t: (key: string, fallback: string) => string): Scenario[] {
  return [
    {
      id: 'party',
      label: t('scenarioParty', 'Plan a birthday party'),
      vaguePrompt: t('partyVaguePrompt', 'Plan a birthday party for me.'),
      vagueResponse: t('partyVagueResponse', "Sure! Here's a general birthday party plan:\n\n1. Choose a venue (your home, a restaurant, or a park)\n2. Pick a theme\n3. Send invitations 2 weeks ahead\n4. Plan food and drinks\n5. Prepare decorations\n6. Organize activities or games\n7. Get a cake\n8. Create a playlist\n\nWould you like me to go into more detail on any of these?"),
      questions: [
        t('partyQ1', "Who's the party for, and how old are they turning?"),
        t('partyQ2', "How many people are you inviting \u2014 a small group or a big bash?"),
        t('partyQ3', "What's the vibe? Chill hangout, themed event, or all-out celebration?"),
        t('partyQ4', "Do you have a budget in mind? Even a rough range helps."),
        t('partyQ5', "Are there any must-haves or deal-breakers? (dietary needs, accessibility, etc.)"),
      ],
      placeholders: [
        t('partyP1', 'e.g., My best friend Mia, turning 16'),
        t('partyP2', 'e.g., About 12\u201315 close friends'),
        t('partyP3', 'e.g., Cozy movie night vibe'),
        t('partyP4', 'e.g., Around $200 total'),
        t('partyP5', 'e.g., One friend is gluten-free, we need it accessible by bus'),
      ],
    ...
    {
      id: 'study',
      label: t('scenarioStudy', 'Study for a history exam'),
      vaguePrompt: t('studyVaguePrompt', 'Help me study for my history exam.'),
      vagueResponse: t('studyVagueResponse', "Here are some tips for studying for a history exam:\n\n1. Review your notes and textbook chapters\n2. Make flashcards for key dates and events\n3. Create a timeline of important events\n4. Practice with past exam questions\n5. Study in short sessions with breaks\n6. Form a study group\n7. Focus on understanding causes and effects\n\nGood luck on your exam!"),
      questions: [
        t('studyQ1', "What period or topic does the exam cover?"),
        t('studyQ2', "What format is the exam \u2014 multiple choice, essays, or a mix?"),
        t('studyQ3', "When is the exam, and how much study time do you have?"),
        t('studyQ4', "What's the part you find trickiest or most confusing?"),
        t('studyQ5', "How does your teacher usually frame questions \u2014 factual recall or analysis?"),
      ],
      placeholders: [
        t('studyP1', 'e.g., The French Revolution through Napoleon'),
        t('studyP2', 'e.g., 30 multiple choice + 2 short essays'),
        t('studyP3', 'e.g., Next Thursday, so about 5 days'),
        t('studyP4', 'e.g., I mix up the different phases and who was in charge when'),
        t('studyP5', 'e.g., She loves "why did X happen" and cause-effect chains'),
      ],
      detailedResponse: (answers) => {
    ...
    {
      id: 'youtube',
      label: t('scenarioYoutube', 'Start a YouTube channel'),
      vaguePrompt: t('youtubeVaguePrompt', 'Help me start a YouTube channel.'),
      vagueResponse: t('youtubeVagueResponse', "Starting a YouTube channel is exciting! Here's how:\n\n1. Choose your niche\n2. Create a Google/YouTube account\n3. Design your channel art and logo\n4. Plan your content calendar\n5. Invest in good equipment (camera, microphone, lighting)\n6. Learn basic video editing\n7. Be consistent with uploads\n8. Engage with your audience\n9. Use SEO and keywords\n10. Be patient \u2014 growth takes time!\n\nLet me know if you want more details on any step!"),
      questions: [
        t('youtubeQ1', "What kind of content excites you? What could you talk about for hours?"),
        t('youtubeQ2', "Who's your dream audience \u2014 who specifically are you making this for?"),
        t('youtubeQ3', "What equipment do you have right now? (phone, computer, etc.)"),
        t('youtubeQ4', "How much time per week can you realistically spend on this?"),
        t('youtubeQ5', "Is there a channel you admire that you'd love your style to feel like?"),
      ],
      placeholders: [
        t('youtubeP1', 'e.g., Explaining science concepts with experiments'),
        t('youtubeP2', 'e.g., Curious teens who think science is boring in school'),
        t('youtubeP3', 'e.g., iPhone 14 and a MacBook, no external mic'),
        t('youtubeP4', 'e.g., About 5-6 hours on weekends'),
        t('youtubeP5', 'e.g., Mark Rober meets Kurzgesagt \u2014 fun but smart'),
      ],
      detailedResponse: (answers) => {
...
const BASE_FLIP_SYSTEM_PROMPT = `You are a friendly planning assistant. The user will state a goal. Do NOT start planning immediately. Instead, ask exactly 5 specific clarifying questions, ONE at a time. After collecting all 5 answers, produce a short, actionable plan.
...
export default function FlipTheScript() {
  const t = useTranslation('flipTheScript');
  const locale = getLocale();
  const langName = languages.find(l => l.code === locale)?.name || 'English';

  const scenarios = buildScenarios(t);
  const FLIP_SYSTEM_PROMPT = locale === 'en'
    ? t('systemPrompt', BASE_FLIP_SYSTEM_PROMPT)
  ...
  const handleSubmitAnswer = () => {
    const answer = currentAnswer || t('skipped', '(skipped)');
    ...
    if (liveMode) {
      ...
      if (isDone) {
        setPhase('result');
        apiMessages.push({ role: 'user', content: t('finalAnswerPrompt', 'That was my 5th answer. Now produce the final plan.') });
      }
    ...
    return (
      <div className="widget-container" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <div style={{ padding: '8px 12px', borderBottom: '1px solid rgba(26,26,46,0.06)', flexShrink: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ flex: 1 }}>
            <span style={{ fontFamily: 'var(--font-heading)', fontSize: '0.95rem', fontWeight: 700, color: '#1A1A2E' }}>{t('title', 'Flip the Script')}</span>
          </div>
        </div>
        ...
        {phase === 'choose' && (
          <div style={{ flex: 1, overflow: 'auto', padding: '10px 12px' }}>
            <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.85rem', color: '#1A1A2E', marginBottom: 12, lineHeight: 1.6 }}>
              {t('chooseIntroMobile', 'Pick a goal. The AI asks 5 questions first.')}
            </p>
            ...
            <div style={{ marginTop: 10 }}>
              <div style={{ display: 'flex', gap: 6 }}>
                <input type="text" value={customGoal} onChange={(e) => setCustomGoal(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && customGoal.trim()) { setMode('freeform'); handleCustomGoal(); } }} placeholder={t('customGoalPlaceholder', 'Or type your own goal...')} style={{ flex: 1, padding: '10px 12px', borderRadius: 8, border: '1px solid rgba(22,199,154,0.25)', background: '#FEFDFB', fontFamily: 'var(--font-body)', fontSize: '16px', color: '#1A1A2E', outline: 'none', minHeight: 40 }} onFocus={(e) => e.currentTarget.style.borderColor = '#16C79A60'} onBlur={(e) => e.currentTarget.style.borderColor = 'rgba(22,199,154,0.25)'} />
                <button onClick={() => { setMode('freeform'); handleCustomGoal(); }} disabled={!customGoal.trim()} style={{ padding: '10px 16px', borderRadius: 8, border: 'none', background: !customGoal.trim() ? 'rgba(26,26,46,0.08)' : '#16C79A', color: '#FFFFFF', cursor: !customGoal.trim() ? 'default' : 'pointer', fontFamily: 'var(--font-body)', fontSize: '0.85rem', fontWeight: 600, minHeight: 40, transition: 'all 0.2s' }}>{t('customGoalButton', 'Go')}</button>
              </div>
            </div>
          </div>
        )}
          ...
          <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column', padding: '10px 12px' }}>
            ...
            {answers.length > 0 && (
              <button onClick={() => { setSheetContent('history'); setSheetOpen(true); }} style={{ padding: '6px 10px', borderRadius: 6, border: '1px solid rgba(26,26,46,0.08)', background: 'transparent', cursor: 'pointer', fontFamily: 'var(--font-mono)', fontSize: '0.7rem', color: '#6B7280', marginBottom: 8, flexShrink: 0, textAlign: 'center' }}>
                {t('viewChatHistory', 'View chat history ({count} Q&A)').replace('{count}', String(answers.length))}
              </button>
            )}
            ...
            {!waitingForQuestion && (
              <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                ...
                <button onClick={handleSubmitAnswer} disabled={isStreaming} style={{ padding: '10px 16px', borderRadius: 8, border: 'none', background: isStreaming ? '#6B7280' : '#0F3460', color: '#FAF8F5', cursor: isStreaming ? 'default' : 'pointer', fontFamily: 'var(--font-body)', fontSize: '0.85rem', fontWeight: 600, minHeight: 44, flexShrink: 0 }}>
                  {answers.length >= 4 ? t('doneButton', 'Done') : t('answerButton', 'Answer')}
                </button>
              </div>
            )}

            <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.7rem', color: '#6B7280', marginTop: 6, textAlign: 'center', flexShrink: 0 }}>
              {t('questionXofY', 'Question {current} of {total}').replace('{current}', String(answers.length + 1)).replace('{total}', '5')}
            </p>
          </div>
        )}
            ...
            {!showComparison ? (
              <>
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.7rem', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase' as const, color: '#16C79A', flexShrink: 0, marginBottom: 8 }}>
                  {t('personalizedResult', 'Your personalized result')}
                </span>
                <div style={{ flex: 1, overflow: 'auto', fontFamily: 'var(--font-body)', fontSize: '0.82rem', lineHeight: 1.7, color: '#1A1A2E', whiteSpace: 'pre-wrap' }}>
                  {(() => {
                    const text = liveMode ? (liveResult || liveResponse || t('generatingMobile', 'Generating...')) : scenario!.detailedResponse(answers);
                ...
                {!isStreaming && (
                  <div style={{ display: 'flex', gap: 6, flexShrink: 0, marginTop: 8 }}>
                    <button onClick={() => { setSheetContent('comparison'); setSheetOpen(true); }} style={{ flex: 1, padding: '10px', borderRadius: 8, border: '1px solid rgba(15,52,96,0.2)', background: 'rgba(15,52,96,0.04)', cursor: 'pointer', fontFamily: 'var(--font-heading)', fontSize: '0.82rem', fontWeight: 600, color: '#0F3460', minHeight: 44 }}>
                      {t('compareResults', 'Compare Results')}
                    </button>
        ...
        {/* BottomSheet */}
        <BottomSheet isOpen={sheetOpen} onClose={() => setSheetOpen(false)} title={sheetContent === 'history' ? t('chatHistoryTitle', 'Chat History') : t('vagueVsSocraticTitle', 'Vague vs. Socratic')}>
            ...
            <div>
              ...
              <div style={{ marginBottom: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                  ...
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.7rem', fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase' as const, color: '#E94560' }}>{t('vaguePromptLabel', 'Vague prompt')}</span>
                </div>
                ...
                <div style={{ fontFamily: 'var(--font-body)', fontSize: '0.8rem', lineHeight: 1.6, color: '#1A1A2E', whiteSpace: 'pre-wrap', opacity: 0.7 }}>
                  {scenario ? scenario.vagueResponse : t('vagueGenericFallbackMobile', "Without clarifying questions, the AI would have given you a generic checklist -- no details about your situation.")}
                </div>
              </div>
              {/* Socratic section */}
              <div style={{ marginBottom: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                  ...
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.7rem', fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase' as const, color: '#16C79A' }}>{t('afterQuestionsLabel', 'After 5 questions')}</span>
                </div>
              ...
              <div style={{ background: 'linear-gradient(135deg, rgba(22,199,154,0.06), rgba(15,52,96,0.06))', border: '1px solid rgba(22,199,154,0.15)', borderRadius: 8, padding: '10px 12px', marginBottom: 12 }}>
                <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.82rem', lineHeight: 1.6, color: '#1A1A2E', margin: 0 }}>
                  <span dangerouslySetInnerHTML={{ __html: t('socraticInsight', '<strong>The Socratic Method works.</strong> Five questions turned a generic checklist into a plan built around <em>your</em> specific situation.') }} />
                </p>
              </div>
            </div>
          )}
    ...
    <div className="widget-container">
      ...
      <div style={{ padding: '1.5rem 2rem', borderBottom: '1px solid rgba(26,26,46,0.06)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <div style={{ flex: 1 }}>
            <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: '1.1rem', fontWeight: 700, margin: 0, lineHeight: 1.3 }}>{t('title', 'Flip the Script')}</h3>
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.75rem', color: '#6B7280', margin: 0, letterSpacing: '0.05em' }}>{t('subtitle', 'What happens when the AI interviews you first?')}</p>
          </div>
        </div>
      ...
      {phase === 'choose' && (
        <div style={{ padding: '2rem' }}>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.95rem', color: '#1A1A2E', marginBottom: '1.5rem', lineHeight: 1.7 }} dangerouslySetInnerHTML={{ __html: t('chooseIntroDesktop', 'Pick a goal. Instead of giving you a generic answer, the AI will ask you <strong>5 clarifying questions</strong> first. Watch how much better the result gets.') }} />
          ...
          <div style={{ marginTop: 12 }}>
            <div style={{ display: 'flex', gap: 8 }}>
              <input type="text" value={customGoal} onChange={(e) => setCustomGoal(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && customGoal.trim()) { setMode('freeform'); handleCustomGoal(); } }} placeholder={t('customGoalPlaceholder', 'Or type your own goal...')} style={{ flex: 1, padding: '10px 14px', borderRadius: 10, border: '1px solid rgba(22,199,154,0.25)', background: '#FEFDFB', fontFamily: 'var(--font-body)', fontSize: '0.9rem', color: '#1A1A2E', outline: 'none', minHeight: 44 }} onFocus={(e) => e.currentTarget.style.borderColor = '#16C79A60'} onBlur={(e) => e.currentTarget.style.borderColor = 'rgba(22,199,154,0.25)'} />
              <button onClick={() => { setMode('freeform'); handleCustomGoal(); }} disabled={!customGoal.trim()} style={{ padding: '10px 20px', borderRadius: 10, border: 'none', background: !customGoal.trim() ? 'rgba(26,26,46,0.08)' : '#16C79A', color: '#FFFFFF', cursor: !customGoal.trim() ? 'default' : 'pointer', fontFamily: 'var(--font-body)', fontSize: '0.85rem', fontWeight: 600, transition: 'all 0.25s', flexShrink: 0, minHeight: 44 }}>{t('customGoalButton', 'Go')} &rarr;</button>
            </div>
          </div>
        </div>
      )}
      ...
      {phase === 'questioning' && (scenario || scenarioId === 'custom') && (
        <div style={{ padding: '1.5rem 2rem' }}>
          ...
          {!waitingForQuestion && (
            <div style={{ display: 'flex', gap: 8 }}>
              <input type="text" value={currentAnswer} onChange={(e) => setCurrentAnswer(e.target.value)} onKeyDown={handleKeyDown} placeholder={!liveMode ? scenario!.placeholders[questionIndex] : t('typeYourAnswer', 'Type your answer...')} style={{ flex: 1, padding: '10px 14px', borderRadius: 10, border: '1px solid rgba(26,26,46,0.1)', background: '#FEFDFB', fontFamily: 'var(--font-body)', fontSize: '0.85rem', color: '#1A1A2E', outline: 'none', minHeight: 44 }} onFocus={(e) => e.currentTarget.style.borderColor = '#0F346040'} onBlur={(e) => e.currentTarget.style.borderColor = 'rgba(26,26,46,0.1)'} />
          ...
          <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.75rem', color: '#6B7280', marginTop: 8, textAlign: 'center' as const }}>
            {t('questionXofY', 'Question {current} of {total}').replace('{current}', String(answers.length + 1)).replace('{total}', '5')} {!liveMode && `\u2014 ${t('pressEnter', 'press Enter to submit')}`}
          </p>
        </div>
      )}
          ...
          {!showComparison ? (
            <>
              <div style={{ marginBottom: '1.25rem' }}>
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase' as const, color: '#16C79A', display: 'block', marginBottom: 8 }}>{t('personalizedResult', 'Your personalized result')}</span>
                <div style={{ fontFamily: 'var(--font-body)', fontSize: '0.85rem', lineHeight: 1.75, color: '#1A1A2E', whiteSpace: 'pre-wrap' as const, maxHeight: dvhValue(40), overflowY: 'auto' as const }}>
                  {(() => {
                    const text = liveMode ? (liveResult || liveResponse || t('generatingPlan', 'Generating your personalized plan...')) : scenario!.detailedResponse(answers);
                    return renderResultText(text);
                  })()}
                  {isStreaming && <span style={{ display: 'inline-block', width: 2, height: '1em', background: '#16C79A', marginLeft: 2, animation: 'pulse 1s infinite' }} />}
                </div>
              ...
              {!isStreaming && (
                <button onClick={() => setShowComparison(true)} style={{ width: '100%', padding: '0.75rem', borderRadius: 10, border: '1px solid rgba(15,52,96,0.2)', background: 'rgba(15,52,96,0.04)', cursor: 'pointer', transition: 'all 0.25s', fontFamily: 'var(--font-heading)', fontSize: '0.9rem', fontWeight: 600, color: '#0F3460' }} onMouseEnter={(e) => e.currentTarget.style.background = 'rgba(15,52,96,0.08)'} onMouseLeave={(e) => e.currentTarget.style.background = 'rgba(15,52,96,0.04)'}>
                  {t('compareDesktop', 'Compare: What would a vague prompt have gotten?')} {'\u2192'}
                </button>
              )}
            </>
          ) : (
            <>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem', marginBottom: '1.25rem' }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                    <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#E94560' }} />
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase' as const, color: '#E94560' }}>{t('vaguePromptLabel', 'Vague prompt')}</span>
                  </div>
                  ...
                  <div style={{ fontFamily: 'var(--font-body)', fontSize: '0.8rem', lineHeight: 1.65, color: '#1A1A2E', whiteSpace: 'pre-wrap' as const, maxHeight: dvhValue(30), overflowY: 'auto' as const, opacity: 0.8 }}>
                    {scenario ? scenario.vagueResponse : t('vagueGenericFallbackDesktop', "Without clarifying questions, the AI would have given you a generic checklist — accurate but impersonal.")}
                  </div>
                </div>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                    <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#16C79A' }} />
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase' as const, color: '#16C79A' }}>{t('afterQuestionsLabel', 'After 5 questions')}</span>
                  </div>
                  <div style={{ background: 'rgba(22,199,154,0.04)', border: '1px solid rgba(22,199,154,0.12)', borderRadius: 10, padding: '1rem', marginBottom: 8, fontFamily: 'var(--font-mono)', fontSize: '0.75rem', color: '#6B7280', fontStyle: 'italic' }}>
                    "{t('sameGoalPlusAnswers', 'Same goal + your 5 answers')}"
                  </div>
              ...
              <div style={{ background: 'linear-gradient(135deg, rgba(22,199,154,0.06), rgba(15,52,96,0.06))', border: '1px solid rgba(22,199,154,0.15)', borderRadius: 10, padding: '1rem 1.25rem', marginBottom: '1rem' }}>
                <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.85rem', lineHeight: 1.65, color: '#1A1A2E', margin: 0 }}>
                  <span dangerouslySetInnerHTML={{ __html: t('socraticInsightDesktop', '<strong>The Socratic Method works.</strong> Five questions turned a generic checklist into a plan built around <em>your</em> specific situation. The AI didn\'t get smarter — it just had better context.') }} />
                </p>
              </div>
              <button onClick={handleReset} style={{ fontFamily: 'var(--font-body)', fontSize: '0.85rem', fontWeight: 600, padding: '0.6rem 1.5rem', borderRadius: 100, border: 'none', cursor: 'pointer', background: '#1A1A2E', color: '#FAF8F5', transition: 'all 0.25s', minHeight: 44 }} onMouseEnter={(e) => e.currentTarget.style.transform = 'scale(1.02)'} onMouseLeave={(e) => e.currentTarget.style.transform = 'scale(1)'}>
                {t('tryAnother', 'Try Another Scenario')}
              </button>
            </>
          )}
        </div>
